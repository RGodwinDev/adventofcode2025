...
void Day7::run() {
	...
	std::unordered_map<int, unsigned __int64> timelines;
	...
	while (getline(file, text)) {
		std::unordered_map<int, unsigned __int64> newTimelines;

		//check all the timelines
		for (std::pair<int, unsigned __int64> const& tl : timelines) {
			//if they hit a splitter, add current timelines to left and right of current.
			if (text[tl.first] == '^') 
			{
	...
	//so just a plain 0 will return int.
	//0ULL returns an unsigned long long.
	//0.0 for double	
